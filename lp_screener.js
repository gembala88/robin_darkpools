...
async function discoverV3Pools(provider, fromBlock, toBlock) {
  ...
  for (let s = fromBlock; s <= toBlock; s += step) {
    ...
    try {
      const logs = await provider.getLogs({ address: V3_FACTORY, topics: [V3_POOL_CREATED], fromBlock: s, toBlock: e });
...
async function discoverV4Pools(provider, fromBlock, toBlock) {
  ...
  for (let s = fromBlock; s <= toBlock; s += step) {
    const e = Math.min(s + step - 1, toBlock);
    try {
      const logs = await provider.getLogs({ address: V4_POOLMANAGER, topics: [V4_INITIALIZE], fromBlock: s, toBlock: e });
      for (const lg of logs) {
...
async function runInitialDiscovery(provider) {
  ...
  // 1. Seed from DexScreener search
...
async function main() {
  ...
  // Config
  const pollMs          = cfgSc('pollIntervalMs') || 1200000; // 20 min default
  const discoveryMins   = cfgSc('discoveryIntervalMin') || 180; // 3 hours default
  const discoveryMs     = discoveryMins * 60 * 1000;
  const summaryMins     = cfgSc('summaryIntervalMin') || 360; // 6 hours default
  ...
  }, Math.min(pollMs, 60000)); // check every 60s or pollMs, whichever is smaller
